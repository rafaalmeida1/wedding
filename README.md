...
Sistema full-stack de lista de presentes de casamento com **Mercado Pago Checkout Transparente** (cartão de crédito, PIX, boleto e cartão de débito virtual Caixa), upload de imagens em Cloudflare R2, e-mails transacionais via Amazon SES, **filas BullMQ no Redis** (pagamentos → estoque + e‑mail) e cache em Redis.
...
- **Backend:** Hono em Node 20 (`@hono/node-server`) + Drizzle ORM + Zod + JWT/bcrypt + **BullMQ** (`bullmq`) + AWS SDK v3 (R2) + Mercado Pago SDK Node (`mercadopago`).
- **Workers:** Processadores BullMQ (filas Redis: pagamento → estoque + e‑mail).
- **Infra local:** Postgres 16 e Redis 7 via `docker-compose`.
...
│  └─ workers/    # BullMQ / jobs em Redis (stock, email, payment)
├─ packages/
...
- **Redis:** `docker exec -it wedding-redis redis-cli`; filas usam prefixo `wedding:jobs` no BullMQ.
...
| **Servidor** | `https://api-casar.bitrafa.com.br` | Hono (`apps/api`) + `apps/workers` + Postgres/Redis |
...
# Mesmo Redis (`REDIS_URL`) para cache, rate-limit **e** filas BullMQ.
REDIS_URL=redis://...
