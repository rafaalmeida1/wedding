import { mutateRefresh } from '@/server/auth-mutations';
...
export async function POST(req: Request) {
  const limited = await rateLimitOrNull(req, '/api/auth/refresh', 60, 60);
  if (limited) return limited;
  const result = await mutateRefresh(ctxFromReq(req));
  if (!result.ok) {
