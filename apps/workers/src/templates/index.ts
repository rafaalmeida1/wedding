...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background:#FBF7F4; padding:32px;">
  <table role="presentation" width="100%" style="max-width:560px; margin:0 auto; background:#fff; border-radius:24px; padding:32px; border:1px solid #FCE4EC;">
    <tr><td>
      ...
      <p style="color:#6E6577; font-size:12px; margin:0;">Lista de Presentes &middot; <a href="${env.APP_URL}" style="color:#C2185B;">${env.APP_URL}</a></p>
    </td></tr>
  </table>
</body>
</html>`.trim();
...
export function renderTemplate(payload: EmailSendPayload): TemplateOutput {
  switch (payload.template) {
    case 'payment-received': {
      ...
      const body = `
        ...
        <p>
          <a href="${env.APP_URL}/dashboard/payments" style="display:inline-block; background:#C2185B; color:#fff; padding:12px 24px; border-radius:999px; text-decoration:none;">Ver no painel</a>
        </p>
      `;
      return {
        subject,
        html: layout('✦ Você foi presenteado!', body),
        text: `${payerName} presenteou vocês com ${productName} (${amount}). ${message ? `Mensagem: ${message}` : ''} Acesse ${env.APP_URL}/dashboard/payments`,
      };
    }
    case 'password-reset': {
    ...
    case 'welcome': {
      ...
      const body = `
        <p style="color:#1F1B24; font-size:16px; line-height:1.6;">Olá, ${name}!</p>
        <p style="color:#1F1B24; font-size:16px; line-height:1.6;">
          Sua lista pública está em <a href="${env.APP_URL}/${username}" style="color:#C2185B;">${env.APP_URL}/${username}</a>.
        </p>
      `;
      return {
        subject,
        html: layout('Bem-vindos!', body),
        text: `Sua lista pública: ${env.APP_URL}/${username}`,
      };
    }
  }
}
